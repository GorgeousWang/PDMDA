...
class Trainer(object):
    ...
    def train(self, dataset,mirnafeaturemat,diseasefeaturemat):
        ...
        for data_i, datain in enumerate(dataset):
            ...
            #### one_disease_feature
            finger, atom_degree_list, bond_feature, bond_degree_list, i_bond_j, adjacency_new = diseasefeaturemat[int(one_diseaseid)]
            ...
            datafeaturein = (finger_tensor,atom_degree_list, bond_feature, bond_degree_list, i_bond_j, adjacency_tensor, mirna_feature_tensor, interaction)
            #end zqc
...
if __name__ == "__main__":
    ...
    dir_input = ('./data/miRNAdisease-HMDD2.0-task1/')

    CV_mirnaid_list = load_strlist(dir_input + 'CVmiRNA_types.txt')
    mirnafeaturemat = load_matrix(dir_input + 'HMDD2_new_miRNAseqfeature.txt')
    n_seqFeature = len(mirnafeaturemat[0,:])
    CV_diseaseid_list = load_strlist(dir_input + 'CVdisease_types.txt')
    ...
    for kk in range(1):
        """Output files."""
        file_output = './output/PDMDA-task1-result.txt'
